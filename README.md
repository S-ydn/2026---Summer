...
- 매 턴 current_query를 별도 필드로 저장하여, 누적된 messages 전체가 아닌 "이번 질문"만 정확히 참조하도록 처리
...
| 요일 기반 상대 표현이 LLM 암산에 의존하여 계산 오류가 드물게 발생 가능 | 파이썬 표준 라이브러리 등을 활용하여 요일(날짜) 계산 tool을 추가 |
